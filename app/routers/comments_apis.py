...
def create_comment(content : str, post_id: int, user_id : int, db : Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
...
def delete_comment(id : int, db : Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
